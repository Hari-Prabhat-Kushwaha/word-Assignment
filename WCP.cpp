...
struct Trie_Node                 
{
    ...
    Trie_Node *take(char ch)
    {
        return nodes[ch - 'a'];
    }
    void update(char ch, Trie_Node *node)
    {
        nodes[ch - 'a'] = node;
        return;
    }
...
public:
    ...
    void initialize_word(string word)          // used to initialize word in trie
    {
        ...
        for (int i = 0; i < word.length(); i++)
        {
            ...
            {
                node->update(word[i], new Trie_Node());
            }
    ...
    void obj(string str, int ind, Trie_Node *temp_node)   // used to reset flag as false
    {
        if (ind == str.length())
        {
    ...
    bool isformed(string str, int ind, Trie_Node *temp_node)   // used to check formed by other words or not
    {
        if (ind == str.length())
        {
